...
async function openEntityForm(entity, item = null, parentId = null) {
    ...
    // Если это создание новой записи (нет item или id)
    if (!item || !item.id) {
        let nextId = 1;
        let prefix = 'Р-';
        ...
        // Логика автопривязки склада и МОЛ для новых приходов (receipts)
        if (entity === 'receipts') {
            // Установка склада по умолчанию (например, первый склад или "Центральный")
            if (!item.warehouse_id && !item.sklad_id) {
                try {
                    const warehousesRes = await fetch('/api/warehouses'); // или /api/sklads в зависимости от вашего API
                    if (warehousesRes.ok) {
                        const warehouses = await warehousesRes.json();
                        const central = warehouses.find(w => w.name && w.name.toLowerCase().includes('центральный')) || warehouses[0];
                        if (central) {
                            item.warehouse_id = central.id;
                        }
                    }
                } catch (err) {
                    console.error('Не удалось загрузить склады для автопривязки', err);
                }
            }

            // Автопривязка текущего пользователя как МОЛ
            const currentUserId = localStorage.getItem('currentUserId');
            if (currentUserId && !item.mol_id) {
                item.mol_id = currentUserId;
            }
        }
    ...
    async function renderField(col) {
        ...
        if (col.field === 'is_posted') {
            ...
            refItems.forEach(refItem => {
                let displayName = refItem.user_fio || refItem.name || refItem.login || refItem.name_full || refItem.title || refItem.doc_number || refItem.gos_number || (`Запись #${refItem.id}`);
    ...
    // Обработчик сохранения формы остается стандартным
    // ...
}
